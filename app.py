...
FINNHUB_API_KEY = "d360nbhr01qumnp3lp3gd360nbhr01qumnp3lp40"
...
@st.cache_data
def auto_fetch_and_save_stock_data(ticker, data_dir="data", period="5y"):
    """
    Automatically fetch stock data from yfinance if CSV doesn't exist
    ...
    if os.path.exists(csv_path):
        try:
            ...
            st.success(f"✅ Loaded existing {ticker_upper} data from CSV")
            return df, csv_filename
        except Exception as e:
            st.warning(f"⚠️ Error loading existing CSV, fetching fresh data: {e}")
    
    # CSV doesn't exist or failed to load - fetch from yfinance
    try:
        st.info(f"📥 Fetching {ticker_upper} data from Yahoo Finance...")
        ...
        # Handle column naming more flexibly
        if 'Adj Close' in df.columns:
            # Keep only the essential columns we need
            df = df[['Date', 'Open', 'High', 'Low', 'Close', 'Volume']]
        else:
            # Fallback: rename all columns if structure is different
            essential_cols = ['Date', 'Open', 'High', 'Low', 'Close', 'Volume']
            df = df.iloc[:, :len(essential_cols)]
            df.columns = essential_cols[:len(df.columns)]
        ...
        st.success(f"✅ Successfully fetched and saved {ticker_upper} data to {csv_filename}")
        st.info(f"📊 Downloaded {len(df)} days of data ({df['Date'].min().date()} to {df['Date'].max().date()})")
        
        progress_bar.empty()
        return df, csv_filename
        
    except Exception as e:
        st.error(f"❌ Failed to fetch {ticker_upper} data: {str(e)}")
        return None, None
...
# NEW: Add ticker input option
input_method = st.sidebar.radio("Select Stock Method", ["Choose from existing CSV files", "Enter ticker symbol"])
...
# NEW: Determine which data to load
df_raw = None
display_name = ""
ticker_guess = ""
...
if input_method == "Choose from existing CSV files":
    ...
    # Load selected CSV (existing functionality)
